jQuery(function ($) {
	...
	$(document).ready(function() {
		...
		mm.add("(min-width: 1024px)", () => {
			ScrollEffects(); // Tier 1: Full motion on Desktop
		});
		mm.add("(min-width: 768px) and (max-width: 1023px)", () => {
			// Tier 2: Reduced motion on Tablets
			ScrollEffects(); 
		});
		mm.add("(max-width: 767px)", () => {
			// Tier 3: Minimal motion on Mobile (Native scrolling, basic fades)
			// ScrollEffects is disabled to save battery and INP.
		});		
		Sliders();	 
		FirstLoad(); 
		PageLoadActions();
...
---------------------------------------------------*/			
	...
	function InitShuffleElements() {	
		...
		shuffleFunctions.applyShuffleEffect(".showcase-portfolio .drawsquill-item .slide-inner", {
			velocity: 30,
			shuffleIterations: 8,
			childSelector: '.slide-title span, .slide-cat span'
		});
		
		shuffleFunctions.applyShuffleEffect(".showcase-gallery .drawsquill-slide", {
			velocity: 30,
			shuffleIterations: 8,
			childSelector: '.slide-title span, .slide-cat span'
		});
...
---------------------------------------------------*/	
	...
	function CleanupBeforeAjax(){		
		...
		DrawsquillSlider.instances.forEach(slider => slider.off());
		DrawsquillSlider.instances = [];
	}
...
---------------------------------------------------*/
	...
	function PageLoad() {		
		...
		gsap.set($(".drawsquill-header"), {yPercent:-50, opacity:0});
		...
		function initOnFirstLoad() {
			...
			imagesLoaded('body', function() {
				...
				gsap.to($(".drawsquill-header"), { duration: 0.45, opacity:1, yPercent:0, delay:0.25, ease: Power2.easeOut, });
	...
	function PageLoadActions() {
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			$('#page-nav .next-ajax-link-page').on('click', function() {	
				...
				$('.drawsquill-header').removeClass('white-header');
				$("#app").remove();
				...
				gsap.to($("#main-page-content, #hero, .drawsquill-footer, .drawsquill-footer-nav"), {duration: 0.3, opacity:0, ease:Power4.easeOut});
				
				if ($("#page-nav").hasClass("move-nav-onload")) {
					$("body").addClass("load-next-page");
					if ($("body").hasClass("smooth-scroll")) {
						var moveNav = $("#content-scroll").height() - ( $("#hero").height() / 2 )  - ( $("#page-nav").height()  ) / 2 - $(".drawsquill-footer").height() / 2	
					} else {
						var moveNav = window.innerHeight - ( $("#hero").height() / 2 )  - ( $("#page-nav").height()  ) / 2 - $(".drawsquill-footer").height() / 2	   
					}
			...
			$('#page-nav .next-ajax-link-page').on('click', function() {					
				...
				gsap.to($(".drawsquill-footer, .drawsquill-footer-nav"), {duration: 0.3, opacity:0, delay:0, ease:Power2.easeInOut});
			});
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			$('.next-ajax-link-project').on('click', function() {
				...
				if ($(".drawsquill-header").hasClass("swapped-logo")) {
					...
					gsap.to($("#drawsquill-logo"), {duration: 0.2, opacity:0, onComplete: function() {
						imgLogoWhite.src = originalSrcWhite;
						imgLogoBlack.src = originalSrcBlack;
						gsap.to($("#drawsquill-logo"), {duration: 0.2, opacity:1});
					}});
				...
				gsap.to($(".drawsquill-footer, .all-works"), {duration: 0.3, opacity:0, ease:Power2.easeInOut});
		...
		if( $('#project-nav').length > 0 ){		
			...
			const pageContent = document.getElementById("drawsquill-page-content");
			...
			ScrollTrigger.create({
				...
				onLeaveBack: function() {
					...
					gsap.set("#drawsquill-logo img, .classic-menu .flexnav li, .button-wrap.menu, .button-icon-link", { clearProps: "all" });
				},			
				scrub: true,
			});	
			...
			if ($("#project-nav").hasClass("change-header")) {
				...
				if (changeMainColor) {
					...
					imagesLoaded('body', function() {
						...
						const blackLogoOpacity = gsap.getProperty('#drawsquill-logo img.black-logo', 'opacity');
						const whiteLogoOpacity = gsap.getProperty('#drawsquill-logo img.white-logo', 'opacity');
						...
						ScrollTrigger.create({
							...
							onEnter: function( st ) {
								...
								gsap.fromTo('#drawsquill-logo img.black-logo', {
								...
								gsap.fromTo('#drawsquill-logo img.white-logo', {
									opacity: whiteLogoOpacity
								}, {
									duration: 1, 
									opacity: blackLogoOpacity,
...
---------------------------------------------------*/	
	...
	function ShowcasePortfolio() {
		...
		gsap.utils.toArray('.drawsquill-item .pixels-cover').forEach((pixelWrapper, index) => {
			const pixelAnimation = pixelWrapper.querySelectorAll(".pixel");
		...
		if( $('.showcase-portfolio').length > 0 ){
			...
			const pageContent = $('#drawsquill-page-content');
			...
			function filter() {				
				...
				var state = Flip.getState('.drawsquill-item');
				var projects = document.querySelectorAll('.drawsquill-item');
				var startHeight = gsap.getProperty(".showcase-portfolio", "height");
				
				var filters = document.querySelectorAll('.filter-option.is_active');
				var parallaxItems = document.querySelectorAll('.showcase-portfolio .drawsquill-item.vertical-parallax .slide-inner');
			...
			if (!isMobile()) {	
				...
				$(".showcase-portfolio .drawsquill-item .slide-inner").on('mouseenter', function() {
					$('#ball p').remove();
					var $this = $(this);			
...
---------------------------------------------------*/
	...
	function ShowcaseHighlights() {
		...
		if( $('.highlights-gallery').length > 0 ){
			...
			var slideElements = $(".drawsquill-slide");
			...
			if (!$('body').hasClass("show-loader")) {
				...
				setTimeout(function() {
					...
					let firstSlide = document.querySelector(".drawsquill-slide");
			...
			slider = new DrawsquillSlider('.highlights-gallery', {
				ease: 0.05,  
				direction: 'vertical', 
				snap: true,
				outer: '.drawsquill-slider',		
				inner: '.drawsquill-slider-viewport',	
				webgl: true,
				...
				on: {	
					init : function(slide) {
						...
						if (firstSlide.classList.contains("change-header")) {
							...
							gsap.to('#drawsquill-logo img.black-logo', { duration: 0.3, opacity: 1, ease:Power2.easeInOut });	
							gsap.to('#drawsquill-logo img.white-logo', { duration: 0.3, opacity: 0, ease:Power2.easeInOut });						
							...
							gsap.to('#drawsquill-logo img.black-logo', { duration: 0.3, opacity: 0, ease:Power2.easeInOut });	
							gsap.to('#drawsquill-logo img.white-logo', { duration: 0.3, opacity: 1, ease:Power2.easeInOut });						
			...
			const arrTitles = gsap.utils.toArray('.drawsquill-caption-wrapper .external-titles .slide-title');
			const arrCategories = gsap.utils.toArray('.drawsquill-caption-wrapper .external-categories .slide-cat');
			...
			if (!isMobile()) {
				...
				$('.drawsquill-slider').on('mousedown', function (evt) {
					$('.drawsquill-slider').on('mouseup mousemove', function handler(evt) {
						if (evt.type === 'mouseup') {					  
							// click
							gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
						...
						$('.drawsquill-slider').off('mouseup mousemove', handler);
					});
				});
				
					
				$('.drawsquill-slider').on('mouseup touchend', function() {
				...
				$(".external-titles .slide-title").on('mouseenter', function() {	
					if (!$('body').hasClass('scale-drag-y')) {
						...
						let relatedSlide = $('.drawsquill-slide[data-slide="' + dataCaption + '"]');
			...
			$('.external-titles .slide-title').on('click', function () {
				...
				let dataCaption = $(this).attr('data-caption'); 
				let parentSlide = $('.drawsquill-slide[data-slide="' + dataCaption + '"]');
				...
				gsap.to('.drawsquill-caption-wrapper', { duration: 0.5, opacity: 0, filter: "blur(5px)", ease: Power4.easeInOut });
...
---------------------------------------------------*/
	...
	function ShowcaseGallery() {
		...
		if( $('.showcase-gallery').length > 0 ){
			...
			gsap.set($(".showcase-gallery .drawsquill-slider .slide-effects"), { opacity: 0 });
			...
			if (!$('body').hasClass("show-loader")) {
				...
				setTimeout(function() {
					...
					gsap.to($(".showcase-gallery .drawsquill-slide .slide-effects"), { duration: 0.5, opacity: 1, delay: 1.1, ease: Power4.easeIn, onComplete: function() {
						$(".showcase-gallery").addClass("active");
					}});
					
					var gallerySlideClasses = [".drawsquill-slide-prev", ".drawsquill-slide-active", ".drawsquill-slide-next"];
					gallerySlideClasses.forEach(function(gallerySlideClass, index) {
						var gallerySlide = $(".showcase-gallery .drawsquill-slider " + gallerySlideClass + " .slide-effects");
			...
			const slidesRoot = document.querySelector('.drawsquill-sync-slider .drawsquill-sync-slider-wrapper .drawsquill-sync-slider-viewport');
			const slidesList = slidesRoot.querySelectorAll('.drawsquill-sync-slider .drawsquill-sync-slide');
			
			let slidesHeight = 0;
			
			for (const drawsquillSyncSlide of slidesList) {
				slidesHeight += drawsquillSyncSlide.offsetHeight;
			}
			...
			if (iterCloning >= 1) {
				...
				for (let i = 0; i < iterCloning; i++) {
					for (const drawsquillSyncSlide of slidesList) {
						let cloneSlide = drawsquillSyncSlide.cloneNode(true);
						slidesRoot.appendChild(cloneSlide);
					}
				}
			}
			
			const drawsquillSyncSlider = document.querySelector('.drawsquill-slider-wrapper.showcase-gallery .drawsquill-sync-slider-viewport');
			const syncSliderClone = drawsquillSyncSlider.cloneNode(true);
			document.querySelector(".drawsquill-slider-wrapper.showcase-gallery .drawsquill-sync-slider-wrapper").appendChild(syncSliderClone);

			
			
			
			slider = new DrawsquillSlider('.drawsquill-slider-wrapper', { 
				...
				on: {	
					init : function(slide) {
						...
						if ($('body').hasClass("show-loader")) {
							...
							imagesLoaded('body', function() {
								...
								gsap.to($(".showcase-gallery .drawsquill-slide .slide-effects"), { duration: 0.5, opacity: 1, delay: 0.7, ease: Power4.easeIn, onComplete: function() {
									$(".showcase-gallery").addClass("active");
								}});
					...
					slideLeaveViewport : function( slide ) {
						gsap.set($('.drawsquill-slider div:not(.drawsquill-slide-visible) .slide-effects'), { x: "" });		
					},
				},
			});			
			
			const syncSliderCloneTranslate = document.querySelectorAll('.drawsquill-slider-wrapper.showcase-gallery .drawsquill-sync-slider-wrapper > .drawsquill-sync-slider-viewport');
			const titleWrapper = document.querySelector('.drawsquill-sync-slider-wrapper');
			
			slider.tl
				.fromTo('.drawsquill-slider-wrapper.showcase-gallery .drawsquill-sync-slider-wrapper', {yPercent: 0 },{ yPercent: -(100 - (100 / syncSliderCloneTranslate.length))},0)
				.fromTo('.hover-reveal', {y: 0}, {y: + (titleWrapper.offsetHeight/2)}, 0)
    

			//Slider Hover Events
			
			$('.drawsquill-sync-slide').on('mouseenter', function() {
				$('.drawsquill-sync-slide').addClass('disable'); 
				$(this).removeClass('disable').addClass('active'); 
			}).on('mouseleave', function() {
				$('.drawsquill-sync-slide').removeClass('disable active'); 
			});
			...
			Array.from(document.querySelectorAll('.drawsquill-sync-slide')).forEach(link => new HoverImgFx(link));
			...
			let gridPreview = new DrawsquillGridPreview( slider, {
			...
			gridPreview.on.beforeShowGrid = () => {
				...
				gsap.to($(".drawsquill-slide.hovered .slide-caption span"), {duration: 0.15, y:-30, opacity:0, stagger:0.05, ease:Power2.easeIn});
				gsap.to($(".drawsquill-slide .slide-caption"), {duration: 0.2, opacity:0, delay:0, ease:Power2.easeIn});
				gsap.to($(".drawsquill-slide .slide-thumb img"), {duration: 0.3, scale:0, opacity:0, ease:Power2.easeIn, onComplete: function() {
					gsap.set(".drawsquill-slide .slide-thumb", { scale: 0, opacity:0 });
				}});
			...
			gridPreview.on.flipShowGrid = (flipstateAllImages, flipstateCurrentImage) => {
				...
				gsap.to(".drawsquill-slider .drawsquill-slide .trigger-item", { duration: 1, opacity:0, scale:0.7, ease:Power2.easeOut });
				...
				if (document.getElementById('drawsquill-page-content').classList.contains('dark-content')) {
					if (gridPreview.currentImage.classList.contains('change-header')) {
						gsap.to('#drawsquill-logo img.black-logo', { duration: 0.5, delay:0.15, opacity: 0, ease:Power2.easeInOut });	
						gsap.to('#drawsquill-logo img.white-logo', { duration: 0.5, delay:0.15, opacity: 1, ease:Power2.easeInOut });						
				...
				} else if (document.getElementById('drawsquill-page-content').classList.contains('light-content')) {
					if (gridPreview.currentImage.classList.contains('change-header')) {
						gsap.to('#drawsquill-logo img.black-logo', { duration: 0.5, delay:0.15, opacity: 1, ease:Power2.easeInOut });	
						gsap.to('#drawsquill-logo img.white-logo', { duration: 0.5, delay:0.15, opacity: 0, ease:Power2.easeInOut });						
			...
			gridPreview.on.beforeHideGrid = () => {
				...
				gsap.to(".drawsquill-slider .drawsquill-slide .trigger-item", { duration: 0.5, opacity:1, scale:1, delay:0.2, ease: 'power3.inOut' });
				...
				gsap.set($(".drawsquill-slide.hovered .slide-caption span"), {y: 30 });				
				gsap.to($(".drawsquill-slide.hovered .slide-caption span"), {
					duration: 0.5,
					y: 0,
					opacity: 1,
					delay: 0.5,
					...
					ease: Power2.easeOut,
					onComplete: function() {
						gsap.set($(".drawsquill-slide .slide-caption span"), { clearProps: "all" });
					}
				});
				gsap.to($(".drawsquill-slide .slide-caption"), {duration: 0.3, opacity:1, delay:0.5, ease:Power2.easeIn});
				gsap.set(".drawsquill-slide .slide-thumb img", { scale: 1, opacity:1, onComplete: function() {
					gsap.to($(".drawsquill-slide .slide-thumb"), {duration: 0.5, scale: 1, delay:0.5, opacity:1, ease: "back.out(2)" });
				}});
			...
			gridPreview.on.flipHideGrid = (flipstateAllImages, flipstateCurrentImage) => {
				...
				Flip.from(flipstateCurrentImage, {
					...
					onComplete: function() {
						...
						const triggeredItem = document.querySelector('.drawsquill-slide.triggered-item');
						if( triggeredItem != null ){
							triggeredItem.classList.remove('triggered-item');
						}
						
						const drawsquillSlides = document.querySelectorAll('.drawsquill-slide');
						drawsquillSlides.forEach(slide => {
						  	slide.style.zIndex = '';
				...
				gsap.to('.header-gradient', { duration: 0.4, backgroundColor: $("#drawsquill-page-content").data("bgcolor"), ease:Power2.easeInOut });
				gsap.to("main", { duration: 0.4, backgroundColor: $("#drawsquill-page-content").data("bgcolor"), ease: Power2.easeInOut });
				
				
				if (document.getElementById('drawsquill-page-content').classList.contains('dark-content')) {
					if (gridPreview.currentImage.classList.contains('change-header')) {
						gsap.to('#drawsquill-logo img.black-logo', { duration: 0.4, opacity: 1, ease:Power2.easeInOut });	
						gsap.to('#drawsquill-logo img.white-logo', { duration: 0.4, opacity: 0, ease:Power2.easeInOut });						
					...
					if (gridPreview.currentImage.classList.contains('change-header')) {						
						gsap.to('#drawsquill-logo img.black-logo', { duration: 0.4, opacity: 0, ease:Power2.easeInOut });	
						gsap.to('#drawsquill-logo img.white-logo', { duration: 0.4, opacity: 1, ease:Power2.easeInOut });						
				...
				gsap.to($("footer .link-text, .drawsquill-pagination, .progress-info, #filters-wrapper"), {duration: 0.3, opacity:1, y:0, stagger:0.05, delay:0.4, ease:Power2.easeInOut});
				gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
				gsap.to('#ball-loader', {duration: 0.2, borderWidth: '4px', top: 0, left: 0});
				$("#ball").removeClass("with-blur");
				$('#ball p').remove();
			...
			if (!isMobile()) {
				...
				$('.drawsquill-slide').on('mouseenter', function() {
					$('.drawsquill-slide').removeClass('hovered');
					$(this).addClass('hovered');
				});
					
				
				$(".showcase-gallery.preview-mode-enabled .drawsquill-slide .slide-inner-height").on('mouseenter', function() {	
				...
				$(".showcase-gallery .drawsquill-sync-slide .trigger-item").on('mouseenter', function() {	
					if (!$('body').hasClass('scale-drag-x')) {
						$('#ball p').remove();
						var $this = $(this);			
						gsap.to('#ball', {duration: 0.3, borderWidth: '2px', scale: 1.4, borderColor:"rgba(170,170,170,0)", backgroundColor:"rgba(170,170,170,0.3)"});
			...
			$('.grid-option').on('click', function () {
				...
				if (!$this.hasClass('active')) {
					...
					$(".drawsquill-sync-slider").removeClass("active");
			
					gsap.to($(".showcase-gallery .drawsquill-sync-slide .slide-title span"), {
						duration: 0.5,
						yPercent: -100,
						opacity: 0,
						stagger: 0.01,
					...
					gsap.to($(".showcase-gallery .drawsquill-slide .slide-effects"), {
						duration: 0.5,
						opacity: 1,
						delay: 0.5,
						ease: Power4.easeIn,
			...
			$('.list-option').on('click', function () {
				...
				if (!$this.hasClass('active')) {
					...
					gsap.set($(".drawsquill-sync-slide .slide-title span"), { yPercent: 100, opacity: 0 });
			
					gsap.to($(".drawsquill-sync-slide .slide-title span"), {
						duration: 0.7,
						yPercent: 0,
						opacity: 1,
						stagger: 0.02,
					...
					setTimeout(function () {
						$(".drawsquill-sync-slider").addClass("active");
					}, 1200);
				}
			});
			...
			$('.trigger-item').on('click', function() {
				...
				gsap.to($(".showcase-gallery .drawsquill-sync-slide .slide-title span"), {duration: 0.3, y: -70, opacity:0, ease:Power2.easeIn});
		...
		if( $('.snap-slider-holder').length > 0 ){
			...
			if (!$("body").hasClass("disable-ajaxload")) {
				$('.snap-slider-thumbs-wrapper .thumb-slide').on('click', function() {
					...
					if (!$("#drawsquill-page-content").hasClass("light-content")) {
						setTimeout(function() {
