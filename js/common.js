	...
	window.ScrollEffects = function() {
		...
		function calculatePixels() {
			...
			if (firstWrapper) {
				...
				if (parentRow) {
					...
					bgColor = document.getElementById('drawsquill-page-content').getAttribute('data-bgcolor');
				}
			} else {
				bgColor = document.getElementById('drawsquill-page-content').getAttribute('data-bgcolor');
			}
		...
		if (isMobile()) {
			...
			function setViewPortHeight() {
				...
				gsap.set(".smooth-scroll main, .has-parallax, .drawsquill-slider-wrapper:not(.content-slider), .showcase-lists .drawsquill-sync-slider, .slider-fixed-content", { height: winHeight});
			}
		...
		if ($('body').hasClass("swap-logo")) {
			...
			ScrollTrigger.create({
				trigger: '.drawsquill-header', // Set the trigger element
				start: 'top 120px',
				onEnter: function() {
				...
				onEnterBack: function() {
					...
					$('.drawsquill-header').removeClass('swapped-logo');
				},
				onLeave: function() {
					...
					$('.drawsquill-header').addClass('swapped-logo');			
				},
		...
		if ($("#hero-interaction").length) {
			...
			$(interactionTarget).mouseenter(function(e) {   
				...
				$("#ball").append('<img class="eyes-icon" src="images/Q_logo1.png" style="transform:scale(0.75);">');                
			
				let imageIndex = 0;
				const images = [
					'<img class="eyes-icon" src="images/Q_logo2.png" style="transform:scale(0.75);">',
					'<img class="eyes-icon" src="images/Q_logo1.png" style="transform:scale(0.75);">' 
				];
		...
		gsap.utils.toArray('.clipped-image-wrapper').forEach((clippedImageWrapper) => {
			...
			function setClippedImageWrapperProperties() {
				...
				gsap.set(clippedImage, { height: window.innerHeight - document.querySelector('.drawsquill-header').offsetHeight * 2 });								
				gsap.set(clippedImageContent, { paddingTop: (window.innerHeight/2) + clippedImageContent.offsetHeight});
				gsap.set(clippedImageWrapper, { height: window.innerHeight + clippedImageContent.offsetHeight - document.querySelector('.drawsquill-header').offsetHeight});
		...
		function setMainPageContentMargins() {
			var winHeight = $(window).height();
			var footerHeight = $('.drawsquill-footer').height();
		...
		gsap.to('.page-nav-caption', {
			...
			scrollTrigger: {
				trigger: '#page-nav',
				start: startValue,
				end: () => `+=${$('#page-nav').outerHeight() + $('.drawsquill-footer').outerHeight()}`,
				scrub: true,
			}			
		});
			...
			imagesLoaded('body', function() {
				setTimeout(function() {
					...
					changeHeaderColor.forEach(function(changeHeaderElement) {						
						var pageHeader = document.querySelector('.drawsquill-header');							
			...
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						...
						if (pageNav) {
							var pageContent = document.querySelector('#drawsquill-page-content');	
		...
		if ($('#drawsquill-page-content').hasClass("dark-content")) {
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						var pageNav = document.querySelector('#project-nav.change-header');
		...
		gsap.set('.header-gradient', {
			delay: 0.5,
			backgroundColor: document.getElementById('drawsquill-page-content').getAttribute('data-bgcolor'),			
			onUpdate: function() {
			var bgColor = document.getElementById('drawsquill-page-content').getAttribute('data-bgcolor');
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to('#drawsquill-logo img.white-logo, #drawsquill-logo img.black-logo', { duration: 0.5, delay:0, opacity: "",  ease: Linear.easeNone, });
...
---------------------------------------------------*/	
	...
	window.FirstLoad = function() {
		...
		if ($("#drawsquill-page-content").hasClass("light-content")) {
			$(".drawsquill-nav-wrapper").css('background-color', function () {
				return $(".drawsquill-header").data('menucolor')
			});
			
			gsap.to("main", {duration: 0.5, backgroundColor: document.querySelector("#drawsquill-page-content").getAttribute("data-bgcolor"), ease:Power2.easeInOut});
			
			$('#magic-cursor').addClass('light-content');
			if( $('#hero').length > 0 ){						
				if( $('#hero').hasClass("has-image")) {	
					$(".drawsquill-header").css('background-color', 'transparent');
				} else {
					if ($(".drawsquill-header").hasClass("fullscreen-menu")) {
						$(".drawsquill-header").css('background-color', 'transparent');
					} else {
						if( $('#blog').length > 0 ){
							$(".drawsquill-header").css('background-color', '#171717');
						}
						if( $('#post').length > 0 ){
							$(".drawsquill-header").css('background-color', '#171717');
						}
			...
			if( $('#hero').length > 0 ){	
				if( $('#hero').hasClass("has-image")) {	
					...
					if ($(".drawsquill-header").hasClass("fullscreen-menu")) {
						...
						if( $('#blog').length > 0 ){
							$(".drawsquill-header").css('background-color', '#fff');
						}
						if( $('#post').length > 0 ){
							$(".drawsquill-header").css('background-color', '#fff');
						}
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to("nav", {duration: 0.3, backgroundColor: document.querySelector("#drawsquill-page-content").getAttribute("data-bgcolor")});
		...
		$('.drawsquill-nav-wrapper .ajax-link').on('click', function() {
		...
		$('#burger-wrapper, .menu .button-text').on('click', function() {
			$('#menu-burger, .drawsquill-nav-wrapper').toggleClass('open');			
			setTimeout( function(){			
				if ($('#menu-burger').hasClass("open")) {
					
					gsap.to('.drawsquill-nav-wrapper', {duration: 0.3, opacity:1, ease:Power2.easeInOut});
					
					$('.drawsquill-header').addClass('over-sidebar').addClass('over-white-section');
					if (!$('#page-content').hasClass("light-content")) {	
						$('#magic-cursor').addClass('light-content');
					}
					if ($('.drawsquill-header').hasClass("invert-header")) {
						$('#header-container').addClass('light-content-header');
					} else {
						$('#header-container').addClass('dark-content-header');
					}
					gsap.set($(".drawsquill-nav-wrapper ul ul li"), {y: 0, opacity:1});
					...
					gsap.to('.drawsquill-nav-wrapper', {duration: 0.3, opacity:0, delay:0.6, ease:Power2.easeInOut});
						
					//Fade Out Navigation Lists						
					gsap.to($(".menu-timeline .before-span"), {duration: 0.5, y:-200, opacity:1, delay:0, stagger:0.05, ease:Power2.easeIn});
					gsap.to($(".drawsquill-nav-wrapper ul ul li"), {duration: 0.5, y:-120, opacity:0, delay:0, stagger:0.03, ease:Power2.easeIn});
					...
					setTimeout( function(){
						...
						$('.drawsquill-header').removeClass('over-sidebar')
						setTimeout( function(){
							$('.drawsquill-header').removeClass('over-white-section');
						} , 350 );
					} , 500 );
				}							
			} , 20 );
				  ...
				  $body.on('mouseup mousemove', function handler(evt) {
					if (evt.type === 'mouseup') {
					  ...
					  $('#itemsWrapperLinks .trigger-item-link, #itemsWrapperLinks .trigger-item-link-secondary').on('click', function() {					
							...
							/*if (!$('#drawsquill-page-content').hasClass("light-content")) {
								...
								if (!$('.portfolio').hasClass("portfolio-shortcode")) {
									if (!parent_item.hasClass("change-header")) {
										$('#drawsquill-page-content').delay(700).queue(function(next){							
							...
							$('.drawsquill-slider-wrapper .trigger-item').each(function(){
							...
							gsap.to('#hero, #show-filters, .item-caption-wrapper, .showcase-portfolio .slide-caption, #page-nav, .drawsquill-footer, .fadeout-element', {duration: 0.5, opacity:0, ease:Power4.easeInOut});
...
---------------------------------------------------*/
	...
	window.Sliders = function() { 
		...
		if( $('.content-slider').length > 0 ){
			...
			slider = new DrawsquillSlider('.content-slider', { 
			...
			$(".slider-button-prev").mouseenter(function(e) {				
				var modifyMouseColor = $('#drawsquill-page-content').attr("data-modify-color");
				if (modifyMouseColor) {
					...
					if ($(this).parents(".drawsquill-slider-wrapper").hasClass("light-cursor")) {
						$("body").addClass("drag-cursor-white");
						gsap.to('#ball', {duration: 0.2, borderWidth: '2px', scale: 1, borderColor:'#fff', backgroundColor:'#fff'});
					} else if ($(this).parents(".drawsquill-slider-wrapper").hasClass("dark-cursor")) {
			...
			$('.content-slider.looped-carousel .drawsquill-slider, .content-slider.small-looped-carousel .drawsquill-slider').on('mouseenter mousemove', function() {	
...
---------------------------------------------------*/
	...
	window.Lightbox = function() {
		...
		function showDetails(item) {
			...
			$('body').prepend(`<div class="drawsquill-img-popup">
				<div class="drawsquill-img-popup-bg-close"></div>				
				<div class="drawsquill-img-popup-viewport">
					<div class="drawsquill-img-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<img />
					<div class="drawsquill-img-popup-close link"></div>						
				</div>				
				<div class="drawsquill-img-popup-prev link"></div>
				<div class="drawsquill-img-popup-next link"></div></div>`);
			
			const details = document.querySelector('.drawsquill-img-popup');
			const detailsBgClose = document.querySelector('.drawsquill-img-popup-bg-close');
			const detailsClose = document.querySelector('.drawsquill-img-popup-close');
			const detailsPreloader = document.querySelector('.drawsquill-img-popup-preloader');
			const detailImage = document.querySelector('.drawsquill-img-popup img');
			const detailPrev = document.querySelector('.drawsquill-img-popup-prev');
			const detailNext = document.querySelector('.drawsquill-img-popup-next');
		...
		function showVideoDetails(event, item) {
			...
			$('body').prepend(`<div class="drawsquill-video-popup">
				<div class="drawsquill-video-popup-bg-close"></div>				
				<div class="drawsquill-video-popup-viewport">
					<div class="drawsquill-video-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<iframe class="drawsquill-video-popup-iframe" frameborder="0" allow="autoplay"></iframe>
					<div class="drawsquill-video-popup-close link"></div>						
				</div>				
				<div class="drawsquill-img-video-prev link"></div>
				<div class="drawsquill-img-video-next link"></div></div>`);
			
			const details = document.querySelector('.drawsquill-video-popup');
			const detailsBgClose = document.querySelector('.drawsquill-video-popup-bg-close');
			const detailsClose = document.querySelector('.drawsquill-video-popup-close');
			const detailsPreloader = document.querySelector('.drawsquill-video-popup-preloader');
			const detailIframe = document.querySelector('.drawsquill-video-popup iframe');
...
---------------------------------------------------*/
	...
	window.Core = function() {
		...
		if (!isMobile() && !$('body').hasClass("disable-cursor")) {
			...
			$(".drawsquill-button .sticky.left").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".drawsquill-button .sticky.right").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".drawsquill-button .parallax-wrap.icon-wrap").mouseenter(function(e) {
			...
			$(".drawsquill-button .sticky").mouseenter(function(e) {
				if ($('#drawsquill-page-content').hasClass("light-content")) {
			...
			$(".drawsquill-button .parallax-wrap").mouseenter(function(e) {
				if ($('#drawsquill-page-content').hasClass("light-content")) {
			...
			//Blog Hover Effects			
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .drawsquill-sidebar-widget a").mouseenter(function(e) {	
				gsap.to('#ball', {duration: 0.2, borderWidth: '1px', scale: 1, opacity:0});
			});	
					
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .drawsquill-sidebar-widget a").mouseleave(function(e) {
				gsap.to('#ball', {duration: 0.3, borderWidth: '4px', scale:0.5, opacity:1});
			});
		}
...
var drawsquill_title = event && event.match(/<title[^>]*>([^<]+)<\/title>/);
if (drawsquill_title && drawsquill_title[1]) {
    $('head title').html(drawsquill_title[1]);
    document.title = drawsquill_title[1];
}
...
var headMatch = event && event.match(/<head[^>]*>([\s\S.]*)<\/head>/i);
if (headMatch && headMatch[0]) {
    newPageHead = document.createElement('head');
    newPageHead.innerHTML = headMatch[0];
